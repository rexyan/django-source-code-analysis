...
class TemplateCommand(BaseCommand):
    ...
    def handle(self, app_or_project, name, target=None, **options):
        ...
        # 校验项目名称是否为空，是否符合规范，是否是已有的模块等
        self.validate_name(name)

        # 如果没有传递 target 目录
        if target is None:
            # 那么就在当前目录创建一个和 name 一样的文件夹
            ...
            # 传递的目录，那么就校验目录是否存在，名称是否符合规范等。
            # os.path.expanduser 用于将包含波浪号（~）的路径扩展为完整的用户路径
            # os.path.abspath 用于返回一个路径的绝对路径
